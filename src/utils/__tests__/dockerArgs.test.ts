...
const { buildDockerShellCommand, buildDockerOpencodeCommand } = await import("../dockerArgs.js");
...
describe("buildDockerShellCommand", () => {
  ...
  it("mounts skills dir", () => {
    ...
    expect(mounts.some((m) => m.startsWith("/home/user/.sith/skills:"))).toBe(true);
  });

  it("mounts opencode config file", () => {
    const args = buildDockerShellCommand("token123");
    const mounts = args.filter((_, i) => args[i - 1] === "-v");
    expect(mounts.some((m) => m.includes("/home/user/.sith/opencode.json:"))).toBe(true);
  });
