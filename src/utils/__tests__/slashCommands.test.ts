...
import { parseSlashCommand, getAvailableCommands } from "../slashCommands.js";
...
describe("parseSlashCommand", () => {
  ...
  it("parses known commands case-insensitively", () => {
    expect(parseSlashCommand("/shell")).toEqual({ type: "shell", raw: "/shell" });
    expect(parseSlashCommand("/SHELL")).toEqual({ type: "shell", raw: "/SHELL" });
    expect(parseSlashCommand("/Shell")).toEqual({ type: "shell", raw: "/Shell" });

    expect(parseSlashCommand("/config")).toEqual({ type: "config", raw: "/config" });
    expect(parseSlashCommand("/help")).toEqual({ type: "help", raw: "/help" });
    expect(parseSlashCommand("/skills")).toEqual({ type: "skills", raw: "/skills" });
  });
