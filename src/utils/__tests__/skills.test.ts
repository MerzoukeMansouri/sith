...
const { getSkillsDir, getOpenCodeConfigPath, isInstalled, findInstructionsFile, installSkill, uninstallSkill } =
  skillsModule;

const expectedSkillsDir = path.join(os.homedir(), ".sith", "skills");
const expectedConfigPath = path.join(os.homedir(), ".sith", "opencode.json");

const validConfig = JSON.stringify({ $schema: "https://opencode.ai/config.json", instructions: [] });
...
describe("getSkillsDir", () => {
  it("returns ~/.sith/skills and creates it", () => {
    ...
    expect(mockFs.mkdirSync).toHaveBeenCalledWith(expectedSkillsDir, { recursive: true });
  });
});
...
describe("getOpenCodeConfigPath", () => {
  ...
  it("removes directory and recreates as file if Docker created a dir", () => {
    ...
    getOpenCodeConfigPath();
    expect(mockFs.rmSync).toHaveBeenCalledWith(expectedConfigPath, { recursive: true, force: true });
...
describe("findInstructionsFile", () => {
  ...
  it("falls through to CAVEMAN.md", () => {
    mockFs.existsSync.mockImplementation((p: string) => p.endsWith("CAVEMAN.md"));
    expect(findInstructionsFile("/skills/caveman")).toBe("CAVEMAN.md");
  });

  it("falls through to instructions.md", () => {
    mockFs.existsSync.mockImplementation((p: string) => p.endsWith("instructions.md"));
  ...
  it("handles nested skills/<name>/SKILL.md structure", () => {
    ...
    expect(findInstructionsFile("/skills/caveman")).toBe(path.join("skills", "caveman", "SKILL.md"));
  });
...
describe("installSkill — builtin", () => {
  ...
  it("adds container instruction path to opencode.json", async () => {
    mockFs.readFileSync.mockReturnValue(validConfig);
    await installSkill(builtinSkill);
    const written = vi.mocked(mockFs.writeFileSync).mock.calls.find(([p]) => p === expectedConfigPath);
    expect(written).toBeDefined();
    const content = JSON.parse(written![1] as string);
    expect(content.instructions).toContain("/root/.opencode/skills/caveman/SKILL.md");
  });
  ...
  it("does not add duplicate instruction", async () => {
    ...
    if (written) {
      const content = JSON.parse(written[1] as string);
      expect(content.instructions.filter((i: string) => i.includes("caveman")).length).toBe(1);
    }
  });
...
describe("installSkill — URL source", () => {
  ...
  it("throws when no source and no builtinInstructions", async () => {
    ...
    await expect(installSkill(noSource)).rejects.toThrow('Skill "x" has no source URL');
  });
  ...
  it("downloads and extracts archive", async () => {
    ...
    expect(mockExeca).toHaveBeenCalledWith("curl", expect.arrayContaining([remoteSkill.source]));
...
describe("uninstallSkill", () => {
  it("removes skill directory", () => {
    ...
    const targetDir = path.join(expectedSkillsDir, "caveman");
    expect(mockFs.rmSync).toHaveBeenCalledWith(targetDir, { recursive: true, force: true });
  });
  ...
  it("removes instruction from opencode.json", () => {
    ...
    expect(written).toBeDefined();
    const content = JSON.parse(written![1] as string);
    expect(content.instructions).not.toContain("/root/.opencode/skills/caveman/SKILL.md");
    expect(content.instructions).toContain("/root/.opencode/skills/other/SKILL.md");
  });

  it("does not rmSync dir if it does not exist", () => {
    mockFs.readFileSync.mockReturnValue(validConfig);
    mockFs.existsSync.mockImplementation((p: string) => p === expectedConfigPath);
