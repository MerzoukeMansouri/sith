...
import { DOCKER_CONFIG, SPINNER_CONFIG } from "../config.js";
...
export function ConfigModal({ onClose, onMessage }: ConfigModalProps): React.ReactElement {
  ...
  async function handlePull(): Promise<void> {
    ...
    try {
      await execa("docker", ["pull", DOCKER_CONFIG.prebuiltImage], { stdio: "inherit" });
      await execa("docker", ["tag", DOCKER_CONFIG.prebuiltImage, DOCKER_CONFIG.imageName], {
        stdio: "inherit",
      });
  ...
  async function handleBuild(): Promise<void> {
    ...
    try {
      ...
      await execa("docker", ["build", "-f", dockerfilePath, "-t", DOCKER_CONFIG.imageName, rootDir], {
        stdio: "inherit",
      });
