...
export function parseNixVersionOutput(stdout: string): boolean {
	...
	const [reqMajor, reqMinor] = NIX_CONFIG.requiredVersion
		.split(".")
		.map(Number);
...
export async function installNix(): Promise<void> {
	...
	if (platform !== "darwin" && platform !== "linux") {
		throw new Error(
			`Unsupported platform: ${platform}. Nix only supports macOS and Linux.`,
		);
	}
	...
	try {
		await execa(
			"sh",
			["-c", `curl -L ${NIX_CONFIG.installerUrl} | sh -s -- --daemon`],
			{
				stdio: "inherit",
			},
		);
	...
	console.log(
		"⚠️  Please restart your shell or run: source ~/.nix-profile/etc/profile.d/nix.sh",
	);
}
